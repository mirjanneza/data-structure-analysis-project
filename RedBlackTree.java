/**
 * An augmented Red-Black Tree generic class
 * that has an added instance variable of size.
 * It can find the kth smallest element and return
 * the size of the tree
 * Base code- Mark Allen Weiss
 */
// RedBlackTree class
//
...
// void printTree( )      --> Print all itemsill
// ******************ERRORS********************************
...
public class RedBlackTree<AnyType extends Comparable<? super AnyType>>
{
    ...
    public void insert ( AnyType item ) throws Exception
    {
        ...
        if( compare( item, parent ) < 0 ){
            parent.left = current;
            //find item to increase the size
            find(item);
    ...
    public AnyType find( AnyType x )
    {
        ...
        for( ; ; )
        {
            if( x.compareTo( current.element ) < 0 ){
                //Increasing the size as we go down
                current.size++;
                current = current.left;
    ...
    private static <AnyType> RedBlackNode<AnyType> rotateWithLeftChild( RedBlackNode<AnyType> k2 )
    {
        ...
        k2.left = k1.right;
        //readjusting size after rotation
